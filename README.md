...
* **Alastria Public-Permissioned blockchain for timestamps and registry for the authoritative credentials** (http://netstats.telsius.alastria.io/). All nodes should be operated inside the EU to facilitate compliance with regulation. It is permissioned but anybody can join, and it is free for non-profits. The network is 100% compatible with Ethereum but much more efficient implementing a PBFT-based consensus algorithm, with strict transaction finality. Typical latency of 1 second and at first glance capacity should be enough (this application is low on writes and heavy on reads). The registry for authoritative credentials can also be stored in that blockchain. Performance should not be a problem, as the queries are local to the blockchain node (there can be many of those nodes accepting queries). Immutability is maximum: essentially, with the strict transaction finality of PBFT-based consensus, even with the whole blockchain network compromised the transaction history in a honest node can not be changed. This, coupled with external "classical" digital signatures makes the system incredibly robust (more than can be achieved with Public-Permissionless networks like Bitcoin or Ethereum or even with "longest-chain-wins" PoA consensus).
